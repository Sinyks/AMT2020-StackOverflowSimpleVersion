package ch.heigvd.amt.project.infrastructure.persistence.jdbc.pgsql;

import ch.heigvd.amt.project.domain.user.IUserRepository;
import ch.heigvd.amt.project.domain.user.User;
import org.jboss.arquillian.container.test.api.Deployment;
import org.jboss.arquillian.junit.Arquillian;
import org.jboss.shrinkwrap.api.ShrinkWrap;
import org.jboss.shrinkwrap.api.spec.WebArchive;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.mindrot.jbcrypt.BCrypt;

import javax.inject.Inject;
import javax.inject.Named;

@RunWith(Arquillian.class)
public class PgsqlUserRepositoryIT {

    private final static String WARNAME = "arquillian-managed.war";

    private final static String USERNAME = "Carl";
    private final static String EMAIL = "Carl@mail.com";
    private final static String PASSWORD = "pass1234";

    private static User newUser = null;

    @Inject
    @Named("PgsqlUserRepository")
    IUserRepository userRepository;

    @Deployment(testable = true)
    public static WebArchive createDeployment() {
        return ShrinkWrap.create(WebArchive.class,WARNAME)
                .addPackages(true, "ch.heigvd.amt")
                .addClass(BCrypt.class);
    }

    @Test
    public void UserRepositorySaveMustAddEntry(){

        newUser = User.builder().username(USERNAME).email(EMAIL).clearTextPassword(PASSWORD).build();

        //userRepository.save(newUser);

    }
}
